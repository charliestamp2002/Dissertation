...
method = "autoencoder"  # Options: "bezier", "autoencoder", "transformer"

# === CHOOSE PLOT MODE ===
plot_mode = "specific_original"  # Options:
...
save_plots = False  # Set to True to save instead of showing
...
comparison_cluster_ids = [5, 12, 23]  # Which original clusters to compare
...
print(f"")
print(f"4. Available original clusters to explore: {sorted([int(x) for x in refined_assignments['base_cluster'].unique() if x.isdigit()])}")

# Show available cluster IDs and their refinement status
print(f"\nCluster Refinement Status:")
for base_cluster in sorted(refinement_stats.index, key=lambda x: int(x) if x.isdigit() else float('inf')):
