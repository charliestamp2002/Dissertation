...
def get_events_in_time_window(events_df, start_time, end_time):
    ...
    mask = (events_df['timestamp'] >= start_time) & (events_df['timestamp'] <= end_time)
...
def classify_run_outcome(post_events, run_end_time):
    """Classify the outcome/consequence of the run"""
    if len(post_events) == 0:
        return 'no_immediate_outcome'
    
    # Look for events within 3 seconds after run
    immediate_events = post_events[post_events['timestamp'] <= run_end_time + 3]
    
    if len(immediate_events) == 0:
        return 'no_immediate_outcome'
    
    # Check for positive outcomes
    for _, event in immediate_events.iterrows():
...
def extract_event_sequence_features(run_df, events_df):
    ...
    # Get run timing - use frameIdx as proxy for timestamp if timestamp not available
    start_time = run_df['frameIdx'].iloc[0]
    end_time = run_df['frameIdx'].iloc[-1]
    ...
    # Extract events in different windows
    pre_events = get_events_in_time_window(events_df, start_time - pre_window, start_time)
    during_events = get_events_in_time_window(events_df, start_time, end_time)
    post_events = get_events_in_time_window(events_df, end_time, end_time + post_window)
    
    # Classify trigger and outcome
    trigger = classify_trigger_event(pre_events)
    outcome = classify_run_outcome(post_events, end_time)
