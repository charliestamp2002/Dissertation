...
def process_runs(players_df, frames_df, runs_list):
    ...
    for _, run_df in grouped:
        ...
        # ALTERNATIVE APPROACH: Mirror team centroid to match run mirroring
        # Uncomment this section if you want to keep all runs in positive Y space
        # 
        # # Apply same mirroring logic to team centroid as was applied to run coordinates
        # run_y_mean = run_df["y"].mean()
        # team_centroid_mirrored = team_centroid.copy()
        # if run_y_mean < 0:  # If run was mirrored, mirror the centroid too
        #     team_centroid_mirrored[1] = -team_centroid_mirrored[1]
        # 
        # # Apply X-flipping to centroid if needed (same as run coordinates)
        # if should_flip_x(team_role, period):
        #     team_centroid_mirrored[0] = -team_centroid_mirrored[0]
        # 
        # # Use mirrored centroid for final coordinates
        # run_df["x_mirror_c"] = run_df["x_mirror"] - team_centroid_mirrored[0]
        # run_df["y_mirror_c"] = run_df["y_mirror"] - team_centroid_mirrored[1]
...
# --------- TACTICAL PHASE FEATURES ---------

def is_build_up_phase(run_df, events_df):
    """Detect if run occurs during build-up play (slow, methodical possession)"""
    if events_df is None or len(events_df) == 0:
        return False
    
    start_frame = run_df['frameIdx'].iloc[0]
    
    # Look at recent events (last 250 frames = ~10 seconds) 
    recent_events = get_events_in_time_window(events_df, start_frame - 250, start_frame)
    
    if len(recent_events) == 0:
        return False
    
    # Build-up indicators: multiple short passes, no recent turnovers
    pass_events = recent_events[recent_events['type'].apply(lambda x: x.get('name') == 'Pass')]
    successful_passes = pass_events[pass_events['pass'].apply(lambda x: x.get('outcome') is None if isinstance(x, dict) else True)]
    
    # Build-up: 3+ successful passes in recent period, low pressure
    build_up_passes = len(successful_passes) >= 3
    low_pressure = len(recent_events[recent_events['type'].apply(lambda x: x.get('name') == 'Pressure')]) <= 1
    
    return build_up_passes and low_pressure

def is_attacking_third_entry(run_df):
    """Detect runs that enter the attacking third"""
    start_x = run_df['x_mirror_c'].iloc[0]
    end_x = run_df['x_mirror_c'].iloc[-1]
    
    # Attacking third roughly x > 17.5 in normalized coordinates
    attacking_third_line = 17.5
    
    return start_x <= attacking_third_line and end_x > attacking_third_line

def is_counter_attack(run_df, events_df):
    """Detect counter-attack situations (rapid transition after regaining possession)"""
    if events_df is None or len(events_df) == 0:
        return False
    
    start_frame = run_df['frameIdx'].iloc[0]
    
    # Look for recent ball recovery events (within 125 frames = ~5 seconds)
    recent_events = get_events_in_time_window(events_df, start_frame - 125, start_frame)
    
    if len(recent_events) == 0:
        return False
    
    # Counter-attack indicators: recent interception/recovery + forward movement
    recovery_events = ['Ball Recovery', 'Interception', 'Tackle']
    has_recovery = any(recent_events['type'].apply(lambda x: x.get('name') in recovery_events))
    
    # Fast forward movement
    dx = run_df['x_mirror_c'].iloc[-1] - run_df['x_mirror_c'].iloc[0]
    is_forward = dx > 5.0  # Significant forward movement
    
    return has_recovery and is_forward

def is_set_piece_context(run_df, events_df):
    """Detect runs occurring during/after set pieces"""
    if events_df is None or len(events_df) == 0:
        return False
    
    start_frame = run_df['frameIdx'].iloc[0]
    
    # Look for recent set piece events
    recent_events = get_events_in_time_window(events_df, start_frame - 125, start_frame + 50)
    
    set_piece_types = ['Corner', 'Free Kick', 'Throw-in', 'Goal Kick']
    return any(recent_events['type'].apply(lambda x: x.get('name') in set_piece_types))

def is_defensive_pressing(run_df, events_df):
    """Detect runs during defensive pressing phases"""
    if events_df is None or len(events_df) == 0:
        return False
    
    # Defensive runs characterized by not being in possession + high pressure context
    in_possession = run_df['in_possession'].iloc[0]
    if in_possession:
        return False
    
    start_frame = run_df['frameIdx'].iloc[0]
    end_frame = run_df['frameIdx'].iloc[-1]
    
    # Look for pressure events during run
    during_events = get_events_in_time_window(events_df, start_frame, end_frame)
    pressure_events = len(during_events[during_events['type'].apply(lambda x: x.get('name') == 'Pressure')])
    
    return pressure_events >= 1

def extract_tactical_phase_features(run_df, events_df):
    """Extract comprehensive tactical phase features"""
    return {
        'build_up_phase': is_build_up_phase(run_df, events_df),
        'attacking_third_entry': is_attacking_third_entry(run_df),
        'counter_attack': is_counter_attack(run_df, events_df),
        'set_piece_situation': is_set_piece_context(run_df, events_df),
        'pressing_phase': is_defensive_pressing(run_df, events_df)
    }

# --------- BALL PROGRESSION FEATURES ---------

def creates_new_passing_option(run_df, ball_carrier_df):
    """Detect if run creates a new passing lane for ball carrier"""
    if ball_carrier_df is None or len(ball_carrier_df) == 0:
        return False
    
    # Simplified: check if runner moves to a more advanced position
    runner_start = run_df[['x_mirror_c', 'y_mirror_c']].iloc[0].values
    runner_end = run_df[['x_mirror_c', 'y_mirror_c']].iloc[-1].values
    
    # More advanced and wider angle = new passing option
    forward_movement = runner_end[0] - runner_start[0] > 3.0
    lateral_movement = abs(runner_end[1] - runner_start[1]) > 2.0
    
    return forward_movement and lateral_movement

def stretches_defensive_line(run_df, ball_carrier_df):
    """Detect if run stretches the defensive line"""
    if ball_carrier_df is None:
        return False
    
    # Runs that go wide or deep behind defense
    runner_end = run_df[['x_mirror_c', 'y_mirror_c']].iloc[-1].values
    
    # Wide stretching: significant lateral movement to wings
    wide_run = abs(runner_end[1]) > 20.0
    
    # Deep run: getting behind defense (advanced position)
    deep_run = runner_end[0] > 25.0
    
    return wide_run or deep_run

def creates_space_for_teammate(run_df, ball_carrier_df):
    """Detect if run creates space by dragging defenders"""
    # Runs that pull defenders away from central areas
    runner_start = run_df[['x_mirror_c', 'y_mirror_c']].iloc[0].values
    runner_end = run_df[['x_mirror_c', 'y_mirror_c']].iloc[-1].values
    
    # Movement away from central corridor
    moves_wide = abs(runner_end[1]) > abs(runner_start[1]) + 3.0
    
    return moves_wide

def provides_support_angle(run_df, ball_carrier_df):
    """Check if run provides good support angle for ball carrier"""
    if ball_carrier_df is None or len(ball_carrier_df) == 0:
        return False
    
    # Simple check: runner moves to supporting position (not too close, good angle)
    runner_pos = run_df[['x_mirror_c', 'y_mirror_c']].iloc[-1].values
    
    # Good support: close enough but not too close, at an angle
    reasonable_distance = 5.0 < np.linalg.norm(runner_pos) < 20.0
    
    return reasonable_distance

def extract_ball_progression_features(run_df, ball_carrier_df):
    """Extract ball progression context features"""
    return {
        'creates_passing_lane': creates_new_passing_option(run_df, ball_carrier_df),
        'stretches_defense': stretches_defensive_line(run_df, ball_carrier_df),
        'creates_space': creates_space_for_teammate(run_df, ball_carrier_df),
        'supports_ball_carrier': provides_support_angle(run_df, ball_carrier_df)
    }
...
def get_events_in_time_window(events_df, start_frame, end_frame, fps=25):
    """Extract events within a specific time window, converting frames to seconds"""
    if events_df is None or len(events_df) == 0:
        return pd.DataFrame()
    
    # Convert frame indices to seconds (assuming 25 fps)
    start_seconds = start_frame / fps
    end_seconds = end_frame / fps
    
    # Convert event timestamps to seconds and filter
    if 'timestamp' in events_df.columns:
        event_seconds = events_df['timestamp'].apply(timestamp_to_seconds)
        mask = (event_seconds >= start_seconds) & (event_seconds <= end_seconds)
        return events_df[mask].copy()
    else:
        return pd.DataFrame()

def classify_trigger_event(pre_events):
    """Classify what event triggered the run"""
    if len(pre_events) == 0:
        return 'unknown'
    
    # Get the most recent event before run
    trigger_event = pre_events.iloc[-1]
    event_type = trigger_event.get('type', {}).get('name', 'unknown')
    
    # Classify trigger types
    if event_type in ['Pass', 'Ball Recovery', 'Interception']:
        return 'ball_recovery'
    elif event_type in ['Dribble', 'Carry']:
        return 'dribble_trigger'
    elif event_type in ['Pressure', 'Foul Won']:
        return 'pressure_trigger'
    elif event_type in ['Ball Receipt*']:
        return 'receipt_trigger'
    else:
        return 'other_trigger'

def classify_run_outcome(post_events):
    """Classify the outcome/consequence of the run"""
    if len(post_events) == 0:
        return 'no_immediate_outcome'
    
    # Check for positive outcomes in post-run events
    for _, event in post_events.iterrows():
        ...
        if event_type == 'Pass':
            ...
            return 'goal_created'
        elif event_type in ['Foul Won', 'Foul Committed']:
            return 'foul_drawn'
    
    return 'possession_continued'

def extract_concurrent_actions(during_events):
    """Extract actions happening during the run"""
    if len(during_events) == 0:
        return {'concurrent_passes': 0, 'concurrent_carries': 0, 'under_pressure': False}
    
    concurrent_passes = len(during_events[during_events['type'].apply(lambda x: x.get('name') == 'Pass')])
    concurrent_carries = len(during_events[during_events['type'].apply(lambda x: x.get('name') == 'Carry')])
    under_pressure = any(during_events['type'].apply(lambda x: x.get('name') == 'Pressure'))
    
    return {
        'concurrent_passes': concurrent_passes,
        'concurrent_carries': concurrent_carries,
        'under_pressure': under_pressure
    }

def assess_defensive_pressure(during_events, run_player_id):
    """Assess defensive pressure during the run"""
    if len(during_events) == 0:
        return {'pressure_events': 0, 'direct_pressure': False}
    
    # Look for pressure events
    pressure_events = during_events[during_events['type'].apply(lambda x: x.get('name') == 'Pressure')]
    
    return {
        'pressure_events': len(pressure_events),
        'direct_pressure': False  # Simplified for now
    }

def extract_event_sequence_features(run_df, events_df):
    """Extract comprehensive event sequence features for a run"""
    if events_df is None or len(events_df) == 0:
        return {
            'triggered_by_event': 'unknown',
            'run_outcome': 'unknown',
            'concurrent_passes': 0,
            'concurrent_carries': 0,
    ...
    # Get run timing using frameIdx
    start_frame = run_df['frameIdx'].iloc[0]
    end_frame = run_df['frameIdx'].iloc[-1]
    run_player_id = run_df['playerId'].iloc[0]
    
    # Define time windows (in frames - roughly 25 fps)
    pre_window = 125   # ~5 seconds before run
    post_window = 75   # ~3 seconds after run
    
    # Extract events in different windows using frame-based timing
    pre_events = get_events_in_time_window(events_df, start_frame - pre_window, start_frame)
    during_events = get_events_in_time_window(events_df, start_frame, end_frame)
    post_events = get_events_in_time_window(events_df, end_frame, end_frame + post_window)
    
    # Classify trigger and outcome
    trigger = classify_trigger_event(pre_events)
    outcome = classify_run_outcome(post_events)
    
    # Extract concurrent actions
    concurrent = extract_concurrent_actions(during_events)
    
    # Assess pressure
    pressure = assess_defensive_pressure(during_events, run_player_id)
    
    # Combine all features
    event_features = {
        'triggered_by_event': trigger,
        'run_outcome': outcome,
        'concurrent_passes': concurrent['concurrent_passes'],
        'concurrent_carries': concurrent['concurrent_carries'],
        'under_pressure': concurrent['under_pressure'],
        'pressure_events': pressure['pressure_events'],
        'direct_pressure': pressure['direct_pressure'],
        'events_in_sequence': len(pre_events) + len(during_events) + len(post_events)
    }
...
def extract_zone_and_run_features(final_runs_df, players_df, frames_df, events_dfs_by_match, metadata_dict):
    ...
    print("Processing player ball tracking data...")
    for _, player_df in tqdm(grouped, desc="Processing players", unit="player"):
    ...
    zone_records = []
    #for run_id, run_df in final_runs_df.groupby("run_id"): BEFORE
    print("Extracting zone and run features for each run...")
    run_groups = list(final_runs_df.groupby("run_id"))
    
    for run_id, run_df in tqdm(run_groups, desc="Processing runs", unit="run"):
        match_id = run_df["match_id"].iloc[0]
        ...
        #Extract event sequence features
        event_features = extract_event_sequence_features(run_df, events_df)
        
        #Extract tactical phase features
        tactical_features = extract_tactical_phase_features(run_df, events_df)
        
        #Extract ball progression features  
        ball_progression_features = extract_ball_progression_features(run_df, ball_carrier_df)
        ...
        zone_records.append({
            ...
            # Event sequence features for tactical context
            "triggered_by_event": event_features['triggered_by_event'],
            "run_outcome": event_features['run_outcome'],
            "concurrent_passes": event_features['concurrent_passes'],
            "concurrent_carries": event_features['concurrent_carries'],
            "under_pressure": event_features['under_pressure'],
            "pressure_events": event_features['pressure_events'],
            "direct_pressure": event_features['direct_pressure'],
            "events_in_sequence": event_features['events_in_sequence'],
            # Tactical phase features
            "build_up_phase": tactical_features['build_up_phase'],
            "attacking_third_entry": tactical_features['attacking_third_entry'],
            "counter_attack": tactical_features['counter_attack'],
            "set_piece_situation": tactical_features['set_piece_situation'],
            "pressing_phase": tactical_features['pressing_phase'],
            # Ball progression features
            "creates_passing_lane": ball_progression_features['creates_passing_lane'],
            "stretches_defense": ball_progression_features['stretches_defense'],
            "creates_space": ball_progression_features['creates_space'],
            "supports_ball_carrier": ball_progression_features['supports_ball_carrier']
        })

    zones_df = pd.DataFrame(zone_records)
    end_time = time.time()
    elapsed = end_time - start_time
    print(f"Zone and run feature extraction completed in {elapsed:.2f} seconds ({elapsed/60:.2f} minutes)")
    return zones_df